...
export const headSections: CitSectionConfig[] = [
  {
    ...
    fields: [
      ...
      field({ field: 'promiseItmes', label: '价格说明', maxLength: 32, span: 6 }),
      field({ field: 'businessItems', label: '业务事项', maxLength: 5, span: 6 }),
    ],
  },
  {
    title: '企业与申报人',
  ...
  },
  {
    ...
    fields: [
      ...
      field({ field: 'trafName', label: '运输工具名称', maxLength: 50 }),
      field({ field: 'billNo', label: '提运单号', maxLength: 32 }),
  ...
  },
  {
    ...
    fields: [
      ...
      field({ field: 'purpOrgCode', label: '目的地海关', type: 'select', optionSource: 'customs', maxLength: 4 }),
      field({ field: 'despDate', label: '启运日期', type: 'date' }),
      field({ field: 'blNo', label: 'B/L号', maxLength: 50 }),
      field({ field: 'specDeclFlag', label: '特种业务标识', maxLength: 10 }),
      field({ field: 'correlationNo', label: '关联号码', type: 'input', maxLength: 500, span: 8 }),
      field({ field: 'correlationReasonFlag', label: '关联理由', type: 'select', optionSource: 'correlationReason', maxLength: 2, span: 8 }),
      field({ field: 'markNo', label: '标记唛码', type: 'textarea', span: 18 }),
...
export const goodsFormFields: CitFieldConfig[] = [
  ...
  goodsField({ field: 'exgNo', label: '货号', maxLength: 30 }),
  goodsField({ field: 'exgVersion', label: '版本号', type: 'number' }),
  goodsField({ field: 'gName', label: '商品名称', maxLength: 255, span: 24 }),
  goodsField({ field: 'gModel', label: '规格型号', type: 'textarea', span: 24 }),
  goodsField({ field: 'goodsSpec', label: '检验检疫货物规格', type: 'textarea', maxLength: 2000, span: 24 }),
  goodsField({ field: 'produceDate', label: '生产日期', type: 'date' }),
  goodsField({ field: 'goodsAttr', label: '货物属性', type: 'select', optionSource: 'goodsAttr', multiple: true, valueChunkSize: 2, maxLength: 20 }),
  goodsField({ field: 'purpose', label: '用途', type: 'select', optionSource: 'purpose', maxLength: 4 }),
  goodsField({ field: 'gQty', label: '成交数量' }),
  goodsField({ field: 'gUnit', label: '成交单位', type: 'select', optionSource: 'unit', maxLength: 3 }),
  goodsField({ field: 'declPrice', label: '申报单价', readonly: true }),
...
export const goodsColumns: CitTableColumn[] = [
  ...
  { title: '货号', dataIndex: 'exgNo', width: 110, ellipsis: true },
  { title: '版本号', dataIndex: 'exgVersion', width: 80, align: 'center' },
  { title: '商品名称', dataIndex: 'gName', width: 180, ellipsis: true },
  { title: '规格型号', dataIndex: 'gModel', width: 220, ellipsis: true },
  { title: '检验检疫货物规格', dataIndex: 'goodsSpec', width: 220, ellipsis: true },
  { title: '生产日期', dataIndex: 'produceDate', width: 180, ellipsis: true },
  { title: '货物属性', dataIndex: 'goodsAttr', width: 140, ellipsis: true },
...
export const relatedTabConfigs: RelatedTabConfig[] = [
  ...
  },
  {
    ...
    parentSource: 'head',
    columns: [
      { title: '申请证书代码', dataIndex: 'appCertCode', width: 180 },
      { title: '正本数', dataIndex: 'applOri', width: 90, align: 'right' },
      { title: '副本数', dataIndex: 'applCopyQuan', width: 90, align: 'right' },
    ],
    formFields: [
      ...compactHeadFields.slice(0, 1),
      { field: 'appCertCode', label: '申请证书代码', required: true, type: 'select', optionSource: 'credential', maxLength: 50 },
      { field: 'applOri', label: '正本数', maxLength: 50 },
      { field: 'applCopyQuan', label: '副本数', maxLength: 50 },
    ],
  },
  {
    key: 'decGoodsLimit',
  ...
  },
  {
    ...
    parentSource: 'head',
    columns: [
      { title: '使用单位联系人', dataIndex: 'useOrgPersonCode', width: 160, ellipsis: true },
      { title: '使用单位联系电话', dataIndex: 'useOrgPersonTel', width: 160 },
    ],
    formFields: [
      { ...compactHeadFields[0], hidden: true },
      { field: 'useOrgPersonCode', label: '使用单位联系人', maxLength: 20 },
      { field: 'useOrgPersonTel', label: '使用单位联系电话', maxLength: 20 },
    ],
  },
  {
    key: 'decMarkLob',
