...
- `jeecg-boot-module/jeecg-module-custom`
  ...
  - Reliable task delivery uses three durable boundaries: a request Outbox written in the task transaction, a result Inbox consumed transactionally, and a Callback Outbox sent after result commit. Never publish RabbitMQ directly from an uncommitted task transaction or invoke customer callbacks inside the result transaction.
  - User routing is grant-based. `CUSTOM_USER_AGENT` controls the agents available to a logged-in user and `CUSTOM_API_APP_AGENT` controls API-app routes. `agentCode`, legacy `companyCode`, and the RabbitMQ routing key represent the same value. Ordinary users without a grant cannot upload; one grant is selected automatically and multiple grants require a choice/default.
  - Business ownership is `customerCode`: users in one customer share documents, tasks and declarations, while all list and direct-ID APIs must hide other customers' data. Do not rely on the globally disabled Jeecg tenant interceptor for this module.
  - Web ZIP upload writes the file, creates the document/internal file/task/request-Outbox records, and returns a queued task in one workflow. The UI polls active states and opens the single-window page only after the declaration import commits.
  - Result consumers must deduplicate by event and source task, validate `runNo`, use bounded retry plus DLQ for poison messages, and update task/document heartbeats. Callback URLs require HTTPS and public-address validation, immutable signed payloads, and bounded retries.
